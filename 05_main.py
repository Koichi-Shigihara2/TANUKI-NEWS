...
def fill_returns(fred=None):
    ...
    for idx, row in need.iterrows():
        ...
        # デバッグ: 最初の3行をログ出力
        if first_rows_logged < 3:
            logger.info(f"fill-returns DEBUG: {row['indicator']} {rd} sp0={t0_val}")
            first_rows_logged += 1
    ...
    logger.info(f"fill-returns: skip_no_sp0={skip_no_sp0} skip_future={skip_future} skip_no_spn={skip_no_spn}")
