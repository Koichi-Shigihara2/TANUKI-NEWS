...
変更点 (v5.0 → v6.0):
  ...
  [監視指標 12本体制]
    手入力:  ISM製造業PMI, ISM非製造業PMI, Conference Board LEI
...
def fill_returns(fred=None):
    """
    events.csv の sp500_t1/t5/t10/t20 と ret_* が空の行を補完。
    sp500_t0 が入っていて sp500_t1 が空の場合のみ処理。
    """
    events = load_events()
    if events.empty:
    ...
    for idx, row in events.iterrows():
        ...
        for n, col_sp, col_ret in [
            ...
            updated += 1
            logger.info(f"[fill-returns] {row['event_id']} t+{n}: {sp_n} ({ret_n:+.2f}%)")
