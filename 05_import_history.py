...
def get_historical_context(fred, target_date: date) -> dict:
    """指定日付の金融環境スナップショットを FRED から取得"""
    ...
    try:
        yc, _ = fred_latest(fred, "T10Y2Y",         target_date, lookback=7)
        hy, _ = fred_latest(fred, "BAMLH0A0HYM2",   target_date, lookback=7)
        vx, _ = fred_latest(fred, "VIXCLS",          target_date, lookback=7)
        ff_hi, _ = fred_latest(fred, "DFEDTARU",     target_date, lookback=30)
        ff_lo, _ = fred_latest(fred, "DFEDTARL",     target_date, lookback=30)

        if ff_hi and ff_lo:
            ctx["ff_rate"] = str(round((ff_hi + ff_lo) / 2, 4))
        if yc: ctx["yc_10y2y"]  = str(round(yc, 4))
        if hy: ctx["hy_spread"]  = str(round(hy, 4))
        if vx: ctx["vix"]        = str(round(vx, 2))
    except Exception as e:
        logger.warning(f"historical context {target_date}: {e}")
